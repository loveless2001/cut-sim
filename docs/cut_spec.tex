...
\author{}
\date{Working spec, v0.4 --- 2026-09-22}
...
\begin{invariant}[\asm]
The \emph{cut-relative} quantities defined in \S2--\S4 (candidate entropic order, local energy flux, registered information) are all \textbf{degenerate on the trivial cut and non-degenerate only for a genuine bipartition}: no complement exists to trace out, no boundary exists across which a flux can be assigned, and no complement channel exists for registration. (The global quantities themselves remain defined --- e.g.\ global entropy is zero, global energy conserved; it is their \emph{cut-relative} versions that degenerate.) This is the one structure invariant under \emph{which} non-trivial cut is chosen: that \emph{some} non-trivial cut is required for these definitions.
\end{invariant}
...
\section{Candidate entropic order \quad{\normalsize\asm\ / \conj}\label{sec:time}}
...
\]
Five distinct quantities or roles must be kept apart, as conflating them is the main source of confusion in \S\ref{sec:spacetime}--\S\ref{sec:lorentz}:
\begin{itemize}[leftmargin=1.4em, itemsep=1pt, topsep=2pt]
  ...
  \item a physical clock reading --- the output of a declared read/count procedure, calibrated against a reference over a stated domain;
  \item proper time --- a relativistic quantity in a domain where the relevant spacetime model applies;
  \item $\tau_A^{\mathrm{ent}}$ --- a candidate \emph{entropic ordering variable}: a monotone reparametrization of $S_A$, defined only on a domain where the specified state and dynamics make $S_A$ monotone enough for that use. \asm/\conj
\end{itemize}
The interpretive claim ``$\tau_A^{\mathrm{ent}}$ is time --- the subsystem's forgetting of its complement'' is therefore model-dependent and conditional, not \est. Monotonicity on an interval can make $S_A$ an ordering variable there; it supplies neither seconds, a universal clock law, nor a mechanism that forms durable records. A globally low-entropy condition does not ensure that the reduced entropy of every cut is monotone.

\begin{proposition}[global purity, local mixedness]{\normalfont\ \est}\label{prop:purity}
If $\lvert\Psi\rangle$ is pure, then $S_U = 0$, while $S_A \ge 0$ with equality iff $A\vert B$ is unentangled. A genuinely entangled part has nonzero reduced entropy $S_A>0$; it can support a cut-relative candidate entropic order only where $S_A$ changes monotonically enough for the declared procedure. Entanglement alone gives $S_A>0$, not a clock. Global purity does \textbf{not} imply stationarity or the absence of internal temporal relations; it only makes this global entropy indicator constant and therefore unusable as a varying clock.
\end{proposition}

\begin{remark}[\gap\ --- direction]
The growth of $S_A$ is \emph{not} derived to be monotone in any absolute sense. Unitarity guarantees reversible evolution; time-reversal symmetry is a separate property of the dynamics and the transformation of states and observables. Given a state and dynamics one can calculate both the magnitude and the sign of $S_A$-change. What the entropy functional alone does not provide is a universal macroscopic arrow. Any monotonicity used to define $\tau_A^{\mathrm{ent}}$ is assumed or checked on a stated domain and requires additional boundary, environmental, and coarse-graining structure (\S\ref{sec:turtles}).
\end{remark}

\subsection*{Four related tasks, kept separate}
\begin{description}[leftmargin=1.6em, style=nextline, itemsep=3pt]
  \item[Ordering and influence.] Add an interaction model, preparations, and control assumptions capable of separating intervention effects from correlations. The model must predict changed-procedure outcomes and need not place all events on one global line.
  \item[Duration and clocks.] Add a physical reference, a read/count procedure, a calibration domain, and a matching rule. The result must reproduce clock relations in its claimed domain, including motion and gravity if that domain is claimed.
  \item[Arrow and records.] Add boundary/preparation conditions, an environment, a coarse-graining, and a mechanism maintaining records. The account must explain historical asymmetry and readback limits rather than defining an arrow and counting it as derived.
  \item[Experience and temporal judgement.] Add a model of trace retention, integration, prediction, and report. Interaction order, record order, and judged order must be distinguished; an information-processing model is not automatically a solution to consciousness.
\end{description}
These are related questions, not four stages automatically deduced in sequence, and they are not asserted to be fully independent.
...
The quantities so far include flows and changing correlations: energy may cross $\hat{H}_{AB}$, records may be created or lost, and $S_A$ may supply a candidate entropic order on a restricted interval. Matter is interpreted below as the \emph{persisting} side; this is a model-dependent proposal, not a definition of time.
...
\section{A candidate entropic ordering and spacetime \quad{\normalsize\conj\ / \gap}\label{sec:spacetime}}

This section explores one \textbf{interpretation inside the quantum realization}; it is not a consequence of the operational core and does not replace established spacetime modelling. A four-manifold and its coordinates are representational and dynamical structures. Writing a coordinate $t$, allowing negative coordinate values, or changing charts does not by itself grant an apparatus the ability to reverse a physical history. Conversely, rejecting a universal entropic clock does not reject relational or relativistic temporal observables.
...
\paragraph{The ``1'': an entropic ordering candidate for a part. \conj}
In the proposal of \S\ref{sec:time}, $\tau_A^{\mathrm{ent}}$ is a monotone reparametrization of $S(\rho_A)$ on a stated domain. It is not identified with every meaning of time. A globally pure state has constant global von Neumann entropy, but this does not make the state stationary or eliminate internal clock relations. Hence the following is a conditional interpretive proposal:

\begin{proposition}[space and time are of different type]{\normalfont\ \asm}\label{prop:diftype}
Within this candidate representation, spatial labels are inherited from an assumed translation structure while $\tau_A^{\mathrm{ent}}$ is a cut-relative ordering constructed from a subsystem entropy. They are introduced by different assumptions. This distinction neither proves that physical space and time have different ontological types nor explains the Lorentzian signature.
\end{proposition}

\paragraph{An indexed-family representation, not an ontological replacement.}
One may organize spatial configurations as a family indexed by the candidate entropic order:
\[
  \mathcal{S}_A \;=\; \big\{\, \Sigma(\tau_A^{\mathrm{ent}}) \,\big\}_{\tau_A^{\mathrm{ent}}},
\]
where each $\Sigma$ is a configuration over the assumed spatial structure. This representation is useful only when the entropy is monotone enough in the declared domain. Its limitations are explicit:
\begin{itemize}[leftmargin=1.4em, itemsep=1pt, topsep=2pt]
  \item A manifold coordinate is not a physical trajectory-control. Coordinate reversal and reversal of a history are different claims.
  \item Monotonicity of $\tau_A^{\mathrm{ent}}$ is assumed or empirically checked over a domain; choosing a monotone index does not explain the origin of irreversibility.
  \item The construction supplies no universal duration scale, clock-calibration law, or guarantee that another cut has the same order.
\end{itemize}
It is therefore a candidate bookkeeping device for a restricted realization, not a derivation of spacetime ontology.

\paragraph{Recovering dimension-like time: emergent local kinematics. \conj}
Observed relativistic physics mixes spatial and temporal coordinates under boosts and uses calibrated clocks and proper time. Relating $\tau_A^{\mathrm{ent}}$ to those quantities requires additional physics. The proposed split is:
\begin{itemize}[leftmargin=1.4em, itemsep=2pt, topsep=2pt]
  \item \textbf{Kinematic level. \conj} A realization would need to construct calibrated local clock readings and show that they join spatial coordinates in the appropriate Lorentz transformations. No such result follows merely by reparametrizing $S_A$.
  \item \textbf{Thermodynamic level. \gap} A macroscopic arrow requires boundary conditions, environment, and coarse-graining. Coordinate covariance neither supplies nor removes that arrow.
\end{itemize}

\begin{proposition}[$3{+}1$ as emergent kinematics]{\normalfont\ \asm/\conj}\label{prop:threeone}
If a physical realization supplies calibrated clocks, a Lorentzian transformation law, and an empirically adequate mapping from those clock readings to $\tau_A^{\mathrm{ent}}$ on a stated domain, then the indexed family can serve as a cut-relative representation of that local kinematics. This spec has not derived that mapping and does not infer it from entropy alone.
\end{proposition}

\begin{remark}[the open joint]{\normalfont\ \conj/\gap}
Proposition~\ref{prop:threeone} is conditional on a missing bridge between an entropic ordering variable, physical clock readings, proper time, and Lorentzian kinematics. The next section (\S\ref{sec:lorentz}) studies one model-dependent obstruction and a tested mechanism; it does not turn this bridge into a result of the operational core.
\end{remark}

\section{Requirements for emergent Lorentz invariance \quad{\normalsize\est\ / \conj}\label{sec:lorentz}}

The ``$3{+}1$'' construction (\S\ref{sec:spacetime}) requires Lorentzian effective kinematics. This section lists the required components and states which are assumed, established in restricted models, or still missing. It does not derive Lorentz invariance.
...
\begin{itemize}[leftmargin=1.4em, itemsep=1pt, topsep=2pt]
  \item a proposed operational core plus a quantum realization assembling partial trace (\S2), Noether (\S3), Shannon quantities and Fields' GHP (\S3--\S4), and an observer-registration condition (\S4);
