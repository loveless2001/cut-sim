...
| Layer | Work | Limit |
|---|---|---|
| **Operational core** | Specifies procedures, records, comparison rules, and the scope of inference | Does not by itself supply dynamics or an ontology of the whole |
...
## 6. Four tasks concerning time

These are related questions. They are neither an automatic sequence of deductions nor
asserted to be fully independent.

| Task | What must be added beyond the core? | Constraint |
|---|---|---|
| **Ordering and influence** | An interaction model, preparations, and control assumptions that distinguish intervention effects from correlation | Predict changes under changed procedures; do not force every event into one global line |
| **Duration and clocks** | A reference system, read/count procedure, calibration domain, and matching rule | Reproduce clock relations in the claimed domain, including motion and gravity when that domain is claimed |
| **Arrow and records** | Boundary/preparation conditions, environment, coarse-graining, and a record-maintenance mechanism | Explain history asymmetry and limits on readback; do not define an arrow and then count it as derived |
| **Experience and temporal judgement** | A model of trace retention, integration, prediction, and report | Distinguish interaction order, record order, and judged order; do not treat an information-processing model as a ready solution to consciousness |
...
- \(\tau_A^{\mathrm{ent}}\): a candidate entropic ordering variable, usable only on a
  stated domain where the relevant entropy is sufficiently monotone.

“Time as forgetting” is therefore a **model-dependent, conditional interpretive
hypothesis**. \(S_A\) is not a universal clock. Monotonicity on an interval can make it
an ordering variable there, but supplies neither seconds, a universal clock law, nor a
mechanism that forms durable records. A globally low-entropy condition does not imply
that every reduced entropy for every cut is monotone.

## 7. Worked examples

### 7.1 Clock matching without absolute time
...
### 7.2 Entropy does not register every change
...
| Arrow conditions | Model assumption | Boundary/preparation condition, environment, and coarse-graining | Tests of entropy histories and record persistence | A universal arrow from the entropy functional alone |
...
## 9. Limits and relation to the current experiments
...
- A coordinate or manifold description does not grant the ability to reverse a
  ...
  domain; it does not explain the asymmetry.
...
## 10. Future work, not a deliverable of this revision
...
separate entropic order from calibrated duration. No new three-state loop experiment or
